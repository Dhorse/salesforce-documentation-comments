...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.commands.registerCommand(
        "force.apexJavadocCompletion",
        (position: vscode.Position) => apexJavadocCompletion(position)
    );
    let provider = {
    ...
    context.subscriptions.push(
        vscode.languages.registerCompletionItemProvider("apex", provider, "*")
    );
}
...
function apexJavadocCompletion(position: vscode.Position): Thenable<boolean> {
    ...
    if (parsed === "") {
        ...
        comment = "\n * $0\n";
        const showTrailing = vscode.workspace
            .getConfiguration()
            .get("force.showApexJavadocTrailingCloseComment");
...
export function parseFunc(startingLine: number, needWhitespace: boolean) {
    ...
    const descriptionTag =
        vscode.workspace
            .getConfiguration()
            .get("force.showApexJavadocDescriptionTag") === true
            ? " @description"
            : "";
    ...
    if (
        vscode.workspace
            .getConfiguration()
            .get("force.showApexJavadocAuthorTag") === true
    ) {
        const authorName = vscode.workspace
            .getConfiguration()
            .get("force.defaultApexJavadocAuthor");
